...
gen Frac_day_logins=(N_day/Span)*100
...
line N_first_users N_last_users date, graphregion(fcolor(white)) graphregion(lcolor(white)) ///
									legend(label(1 "Primo Login") label(2 "Ultimo Login")) legend(size(small))  legend( ring(0) position(1) cols(1)) ///
...
gduplicates drop date, force
graph box N_users, over(Day, relabel(1 "Dom" 2 "Lun"  3 "Mar" 4 "Mer" 5 "Gio" 6 "Ven" 7 "Sab")) noout medtype(cline)  medline(lcolor(red)) ///
...
graph box N_users, over(hour) noout  medtype(cline)  medline(lcolor(red))  graphregion(fcolor(white)) graphregion(lcolor(white))
...
su date, meanonly
heatplot Obs ID date, colors(magma, reverse)  ybins(2000)  xlabel(`r(min)'(31)`r(max)', format(%td) labsize(vsmall) angle(45)) graphregion(fcolor(white)) graphregion(lcolor(white))
...
graph bar Fraction, over(province, label(labsize(small) ) sort((mean) Fraction))  graphregion(fcolor(white)) graphregion(lcolor(white)) ytitle("")
...
bysort amount mcc: gen ind_amount_mcc=_n
order amount mcc sd_amount_mcc
gduplicates drop amount mcc, force

replace sd_amount_mcc=26000 if sd_amount_mcc>26000
heatplot sd_amount_mcc amount mcc if amount<200, colors(magma, reverse) 
order amount mcc sd_amount_mcc
drop if sd_amount_mcc==.
*/

graph box carbonEmissionInGrams if N_Tot>100, over(mcc) noout
*/
...
gduplicates drop date, force
su date, meanonly
line Tot_Outflow Tot_Inflow date , graphregion(fcolor(white)) graphregion(lcolor(white)) ///
									legend(label(1 "Out") label(2 "In")) legend(size(small))  legend( ring(0) position(1) cols(1)) ///
									lpattern(solid solid) scheme(s1color) lcolor("8 81 156"  "146 0 0") ///
									xtitle(" ")  ytitle("Total Euros")  xlabel(`r(min)'(31)`r(max)', format(%td) labsize(vsmall) angle(45)) 
